...
const Header = () => {
  ...
  const createReturnLink = (returnPath: string): string => {
    return `/api/auth/login?returnTo=${encodeURIComponent(returnPath)}`;
  };
  const returnLink = createReturnLink('/dashboard');
  ...
  return (
    <>
          ...
          <div className="relative z-30 mx-auto flex w-full items-center justify-between px-4 sm:px-6 py-4 xl:max-w-7xl xl:px-0">
            ...
            <div className="hidden xl:flex flex-row items-center gap-4 justify-center mx-auto">
              {navItems.map((item) => (
                <div
                  key={item.id}
                  className="font-label-size-200 group flex items-center gap-1 font-semibold"
                >
      ...
      {!user && (
        <button
          onClick={handleLogin}
          className="font-button-size-100 px-4 py-2 bg-gradient-to-br from-gradient-vibrant-blue-100 to-gradient-vibrant-blue-200 text-white-950 rounded-lg transition-all hover:shadow-gray-lg"
        >
          Sign up / Login
        </button>
      )}
          ...
          Logout
            ...
            >
              <div className="relative w-6 h-6">
                <span
                  className={`absolute h-0.5 w-full bg-gray-700 transform transition-all duration-300 ease-in-out ${
